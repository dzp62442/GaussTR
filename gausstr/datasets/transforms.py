...
def _as_bytes(data):
    ...
    if isinstance(data, np.ndarray):
        return data.tobytes()
    raise TypeError(f'Unsupported image bytes type: {type(data)}')
...
@TRANSFORMS.register_module()
class BEVLoadMultiViewImageFromShards(BEVLoadMultiViewImageFromFiles):
    """Load multi-view images from `raw_nuscenes` shard payloads."""

    def transform(self, results: dict) -> Optional[dict]:
        filename, cam2img, lidar2cam, cam2ego = [], [], [], []
        raw_images = results['_sharded_raw']['images']
        for cam_name, cam_item in results['images'].items():
            ...
            if cam_name not in raw_images:
                raise KeyError(
                    f'Sharded raw image payload missing camera {cam_name}.')
        ...
        img_bytes = [
            _as_bytes(raw_images[cam_name]['bytes'])
...
@TRANSFORMS.register_module()
class LoadShardedOccFromArrays(BaseTransform):

    def __init__(self, group='occ_gt'):
        self.group = group
...
@TRANSFORMS.register_module()
class LoadShardedFeatMaps(BaseTransform):

    def __init__(self, group, key, apply_aug=False):
        self.group = group
        self.key = key
        self.apply_aug = apply_aug
